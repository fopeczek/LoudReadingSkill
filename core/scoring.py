...
class TotalScore:
    ...
    correct: float = 0.
    incorrect: float = 0.
    total_questions: float = 0.

    @staticmethod
    def FromDict(d: dict):
        return TotalScore(d["accuracy"], d["speed"], d["correct"], d["incorrect"], d["total_questions"])

    def dict(self):
        return {"accuracy": self.accuracy, "speed": self.speed, "correct": self.correct, "incorrect": self.incorrect,
...
class Scoring:
    ...
    def load_answers(self):
        self._answers = {sentence: Score.FromDict(score) for sentence, score in
                         create_and_load_file(self._answers_file, {}).items()}
        self._scores_sort = [(score, sentence) for sentence, score in self._answers.items()]
